/* ============================================================================
   ...
   Supports flat prices, Small/Medium/Large sizes, weight tiers (mithai),
   and Half/Full portions — whatever is listed in an item's "options".
   ============================================================================ */
...
(function () {
  ...
  // A short "Small ₹149 · Medium ₹249 · Large ₹399" summary shown in the row
  function priceSummary(options) {
    return options
      .map(function (o) {
        return (o.label ? o.label + ' ' : '') + formatPrice(o.price);
      })
      .join(' &middot; ');
  }
  ...
  function buildItemRow(item) {
    var note = item.note
      ...
      : '';
    var optionsJson = escapeHtml(JSON.stringify(item.options));
    return (
      '<div class="menu-row">' +
        '<div class="item-info">' +
        ...
        '<div class="item-action">' +
          '<span class="item-price">' + priceSummary(item.options) + '</span>' +
          '<button type="button" class="btn-order-now" data-dish="' + escapeHtml(item.name) + '" data-options="' + optionsJson + '">' +
